...
function CustomerTable() {
  ...
  const filteredItems = customers.slice(firstIndex, lastIndex);

  useEffect(() => {
    axios
      .post('http://49.207.11.223:3307/clients/getTotalCustomerWiseSegementation', {
        clientId: 1,
        zoneId: 0,
        dmaId: 0
      })
...
export default CustomerTable;
 ...
 function Paginations({ customers, itemsPerPage, currentPage, setCurrentPage }) {
  // const { zonesList, itemsPerPage, currentPage } = useStateContext();
  const items = Math.ceil(customers.length / itemsPerPage);
  const paginateItems = Array(items)
    .fill()
    .map((_, index) => index + 1);

  return (
    <Row style={{ position: 'absolute', bottom: -10, width: '100%', overflow: 'scroll' }}>
